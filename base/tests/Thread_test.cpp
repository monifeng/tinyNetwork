...
class Foo
{
 public:
  ...
  void memberFunc()
  {
    printf("tid=%d, Foo::x_=%f\n", tnet::CurrentThread::tid(), x_);
  }

  void memberFunc2(const std::string& text)
  {
    printf("tid=%d, Foo::x_=%f, text=%s\n", tnet::CurrentThread::tid(), x_, text.c_str());
  }
...
int main()
{
  ...
  tnet::Thread t2(std::bind(threadFunc2, 42),
  ...
  Foo foo(87.53);
  tnet::Thread t3(std::bind(&Foo::memberFunc, &foo),
                   "thread for member function without argument");
  t3.start();
  t3.join();

  tnet::Thread t4(std::bind(&Foo::memberFunc2, std::ref(foo), std::string("Shuo Chen")));
  t4.start();
  t4.join();
