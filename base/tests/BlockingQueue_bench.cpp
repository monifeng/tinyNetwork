...
class Bench
{
 public:
    ...
    : latch_(numThreads)
  {
    ...
    for (int i = 0; i < numThreads; ++i)
    {
      ...
      threads_.emplace_back(new tnet::Thread(
            std::bind(&Bench::threadFunc, this), tnet::string(name)));
    }
  ...
  void joinAll()
  {
    ...
    {
      queue_.put(tnet::Timestamp::invalid());
    }
 ...
 private:
  ...
  void threadFunc()
  {
    ...
    while (running)
    {
      tnet::Timestamp t(queue_.take());
      tnet::Timestamp now(tnet::Timestamp::now());
      if (t.valid())
      {
        int delay = static_cast<int>(timeDifference(now, t) * 1000000);
        // printf("tid=%d, latency = %d us\n",
        //        tnet::CurrentThread::tid(), delay);
        ++delays[delay];
  ...
  tnet::BlockingQueue<tnet::Timestamp> queue_;
  tnet::BlockingQueue<int> delay_queue_;
